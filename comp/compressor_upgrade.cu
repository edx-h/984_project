...
// Lookup table on device: -1 means not in dictionary, otherwise index in d_dictionary
__device__ int d_dictMap[256];

// **Compression Kernel with Coalesced Memory Access and Lookup Table**
                                                ...
                                                unsigned char *d_tempOut, int *d_chunkBitSizes)
{
    ...
    for (int i = threadIdx.x; i < n; i += blockDim.x) {
        ...
        int dict_index = d_dictMap[symbol]; // Direct lookup
        bool found = (dict_index != -1);
    ...
    if (threadIdx.x == 0) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            int bits;
            if (found) {
                code = ((1 << 4) | (dict_index & 0xF)); // 5位：1(标志位)+4位索引
                bits = 5;
            } else {
                code = (0 << 8) | symbol; // 9位：0(标志位)+8位原始字符
                bits = 9;
            }
            int remaining = bits;
            while (remaining > 0) {
...
// **Non-Coalesced Compression Kernel (unchanged)**
                                                   ...
                                                   unsigned char *d_tempOut, int *d_chunkBitSizes)
{
    ...
    int tid = threadIdx.x;
    int start_idx = (tid * 17) % total_threads;
...
// **Compute Offsets Kernel (unchanged)**
...
// **Parallel Assembly Kernel (unchanged)**
...
// **Serial Assembly Kernel (unchanged)**
...
// **Decompression Kernel (unchanged)**
...
// **Process File Function (modified to include lookup table setup)**
void processFile(const char* filePath, int switchValue, long long& total_duration, 
    ...
    // 新增主机端字典
    const unsigned char h_dictionary[DICT_SIZE] = { ' ', 'e', 't', 'a', 'o', 'i', 'n', 's', 'r', 'h', 'l', 'd', 'u', 'c', 'm', 'f' };
    // 将主机字典拷贝到设备常量内存
    cudaMemcpyToSymbol(d_dictionary, h_dictionary, DICT_SIZE * sizeof(unsigned char));

    // 构建查找表
    int h_dictMap[256];
    for (int i = 0; i < 256; i++) h_dictMap[i] = -1; 
    for (int d = 0; d < DICT_SIZE; d++) {
        h_dictMap[h_dictionary[d]] = d; // 使用主机端字典
    }
    // 拷贝查找表到设备端
    cudaMemcpyToSymbol(d_dictMap, h_dictMap, 256 * sizeof(int));
